...
interface DeletedItemsComponentProps {
    deletedItems: GroupedItem[];
    onRestoreItem: (item: Item) => void;
}

const DeletedItemsComponent: React.FC<DeletedItemsComponentProps> = ({deletedItems, onRestoreItem}) => {
    ...
    const handleDeleteItem = (itemId: number) => {
        const updatedItems = deletedItems
            .map((group) => ({
                ...group,
        ...
        onRestoreItem(updatedItems as unknown as Item);
    };
    ...
    return (
        <>
            ...
            <div
                className="deleted-items-container"
                style={{
                    height: "100%",
                    overflowY: "auto",
                    padding: "20px",
            ...
            >
                {deletedItems.length > 0 ? (
                    <div
                        className="view-que-list scroll-inner"
                        style={{overflowY: "auto", maxHeight: "60vh"}}
                    >
                        {deletedItems.map((group, groupIndex) => (
                            <div
                                key={`deleted-passage-${group.passageId}-${groupIndex}`}
                            ...
                            >
                                ...
                                {group.items.map((item, index) => (
                                    <div
                                        key={`deleted-item-${item.itemId}-${index}`}
